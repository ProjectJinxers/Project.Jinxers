...
/**
 * Test class for connecting to ipfs.infura.io. The only two add operations that didn't return 500, 403 or 401
 * are root add and block put. Neither of them required authentication. Currently dag put doesn't work. With valid
 * credentials for an Ethereum project, the server returns 403. With sign in credentials or credentials for a FileCoin
 * project, it returns 401. In the dashboard for the Ethereum project the dag put requests are listed as (blank). The
 * root add requests are listed as /api/v0/add.
 * 
 * The tests only work, if the ipfs.infura.io node is configured in config.yml. You can rename or copy the file
 * config-infura.yml. Infura secrets must be configured in test-secret-config.yml, which is obviously not pushed to
 * version control. The structure is: infura at root level and 'user' as well as 'pass' as children.
 * 
 * The current save test doesn't actually save. The hash for reading the object is what the save operation, if it
 * worked, would return. Reading (dag get) didn't work at first (read timeout), but after a little while, the
