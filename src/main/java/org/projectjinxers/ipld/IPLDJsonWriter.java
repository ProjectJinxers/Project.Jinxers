...
public class IPLDJsonWriter implements IPLDWriter {
	...
	private void writeSignature(ECDSASignature signature) throws IOException {
		jsonWriter.name(IPLDJsonReader.KEY_SIGNATURE).beginObject()
		.name(IPLDJsonReader.KEY_SIGNATURE_R).value(signature.r)
		.name(IPLDJsonReader.KEY_SIGNATURE_S).value(signature.s)
