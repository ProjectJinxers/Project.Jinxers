...
/**
 * The base interface for all data model classes, that can be saved as IPLD in
 * IPFS.
 * 
...
public interface IPLDSerializable {
     ...
     * @param contextForRecursion if non-null, links should be resolved with the
     *                            help of this context (not with the given reader!)
     */
     ...
     * {@link #writeProperties(ECKey, IPLDContext)} and, if that returned a non-null
     * hash, signs that hash with the given key.
     * 
    ...
    /**
     * @return the hash of the serialized data (this is not the multihash for the
     *         object)
     */
    byte[] hash();
