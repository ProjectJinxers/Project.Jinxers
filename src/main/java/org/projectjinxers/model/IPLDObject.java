 ...
 * By default, loading/reading an instance from IPFS does not automatically load the entire (sub)tree. Primitive kinds
 * are mapped. Links are resolved on access. 
 * 
...
public class IPLDObject<D extends IPLDSerializable> {
	...
	public boolean verifySignature(byte[] publicKey) {
		return signature != null && getMapped() != null
				&& ECKey.verify(mapped.hash(), signature, publicKey);
	}
